...
export default function Submit() {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    const { error: subErr } = await supabaseRef.current.from('submissions').insert({
    ...
    const { error: updErr } = await supabaseRef.current.from('assignments').update({ status: 'submitted' }).eq('id', assignment);
    setLoading(false);
  ...
  return (
    <form onSubmit={submit} className="max-w-xl space-y-4">
      ...
      <input className="w-full border p-3 rounded-lg" placeholder="Paste link (GitHub, Drive, etc.)"
             value={url} onChange={(e)=>setUrl(e.target.value)} required />
      <textarea className="w-full border p-3 rounded-lg" rows={5} placeholder="Notes (optional)"
